...
public class MrClient : BaseClient
{
    ...
    /// <param name="token"></param>
    /// <returns></returns>
    public Task<GazetteerRecord[]?> GetGazetteerRelations(int mrgId, MrRelationDirection direction = MrRelationDirection.Upper, MrRelationType type = MrRelationType.PartOf, CancellationToken token = default)
    {
        var parameters = new Dictionary<string, object?>
        {
            {"direction", direction},
